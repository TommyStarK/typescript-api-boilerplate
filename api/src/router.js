...
// First path handled
router.get(`/${config.app.url}`, (request, response, next) => response.status(200).json({
  status: 200,
  success: true,
...
// Auth required route
router.get(`/${config.app.url}/hello`, (request, response, next) => response.status(200).json({
  status: 200,
  success: true,
